...
export interface CalcParams {
  ...
  hAllowance: number; vAllowance: number
...
function LengthInput({
  label, valueMeters, unit, hint, onChangeMeters, onUnitChange,
}: {
  label: string; valueMeters: number; unit: string; hint?: string
  ...
  return (
    <div>
      ...
      <div ref={ref} className="relative">
        ...
        {open && (
          <div className="absolute right-0 top-[calc(100%+6px)] z-50 w-[160px] rounded-2xl border border-gray-100 bg-white py-2 shadow-[0_8px_32px_rgba(0,0,0,0.12)] animate-scale-in">
            {groups.map((group, gi) => (
              <div key={group}>
...
export default function RouteConditionsCard({ value, onChange, fromVal, toVal, onFromChange, onToChange }: Props) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-card overflow-hidden h-full hover:shadow-card-hover transition-shadow duration-200">
      <div className="flex items-center gap-2 px-5 pt-5 pb-4">
        <div className="w-1 h-5 rounded-full bg-[#2D98E9]" />
        <h3 className="text-[13px] font-semibold text-gray-900">Route and Conditions</h3>
      </div>
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <LengthInput
            label="Horizontal" valueMeters={value.horizontal} unit={hUnit}
            hint="Horizontal distance in the tray/trench."
            onChangeMeters={v => set('horizontal', v)} onUnitChange={setHUnit}
          />
          <LengthInput
            label="Vertical" valueMeters={value.vertical} unit={vUnit}
        ...
        <div className="grid grid-cols-2 gap-3">
          <NumUnit label="Allowance (%)" value={value.hAllowance} unit="%"
            hint="Suggestion: 5–15% (straight → 5%, complex → 15%)."
            onChange={v => set('hAllowance', v)} />
          <NumUnit label="Allowance (%)" value={value.vAllowance} unit="%"
            hint="Suggestion: 5–15% (straight → 5%, complex → 15%)."
            onChange={v => set('vAllowance', v)} />
        </div>

        <NumUnit label="Tolerance (%)" value={value.tolerance} unit="%"
